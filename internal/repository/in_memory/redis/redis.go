/*
Package redis.
Пакет для взаимодействия с in memory хранилищем Rеdis. Функция MustCreate возвращает структуру, содержащую методы,
удовлетворяющие интерфейсу in_memory.Interface и содержащую пул соединений с redis-сервером. При невозможности
установить соединение, работа приложения останавливается. Для работы приложения в настройках redis Access Control List
должны быть установлены разрешения на выполнение данным приложением операций SET, GET, HSET, HGETALL.
*/
package redis
